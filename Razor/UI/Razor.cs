...
namespace Assistant
{
  public partial class MainForm
  {
    ...
    private class MyLoader : DocumentLoader
    {
      ...
      private void execScriptv2_Click(object sender, EventArgs e)
    {
...

      {
        ...
        try
        {
          var x = new ClearScriptBinding.Player(engine);
          //engine.AccessContext = typeof(ClearScriptBinding.Player);
          //engine.AddRestrictedHostObject<ClearScriptBinding.Player>("player", ,x); ;
          engine.AddHostObject("player", x);

          // engine.AddHostObject("backpack", new ClearScriptBinding.Backpack());

          //  x.Finalize();
          //          object o = engine.Evaluate(@"(
          //class Plugin {
          //constructor() { }
          //}
          //).valueOf()");

          //engine.DocumentSettings.Loader.
          engine.DocumentSettings.AccessFlags = DocumentAccessFlags.EnableFileLoading;

          engine.AddHostObject("host", new ExtendedHostFunctions());

          engine.DocumentSettings.Loader = new MyLoader();

          //engine.AddHostType("Plugin", typeof(ClearScriptUtils.Plugin));
          object result = engine.Evaluate(new DocumentInfo { Category = ModuleCategory.Standard }, scriptv2Editor.Text);
          execScriptResult.Text = result.ToString();
        }
        catch (Exception ex)
        {
          execScriptResult.Text = ex.ToString();
